...
(function (root, factory) {
     ...
     * @return {HTMLElement}
     */
    Rows.prototype.build = function (row,button_counter) {
        var td, tr = createElement("tr");

        var headings = this.dt.headings;
        var check_box = createElement("input");
        ...
        if (headings.length === 4) {
            row.push('<input type="checkbox" onclick=add_class_selected() class="checkbox">');
        }
        each(headings, function (h, i) {
            ...
            // Fixes #29
            if (headings[2] !== 'Booleans' && headings.length === 3) {
                if (row[i] === undefined) {
                    row[i] = '<button type="button" id="btn' + button_counter
                        + '" onclick="download_files()" data-columns="'
                        + button_counter + '"> Download </button>'
        ...
        if (isArray(data)) {
            ...
            if (isArray(data[0])) {
                ...
                each(data, function (row, i) {
                    dt.data.push(this.build(row,button_counter));
